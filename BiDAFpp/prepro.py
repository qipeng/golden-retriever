...
from util import NUM_OF_PARAGRAPHS, MAX_PARAGRAPH_LEN

import torch
from stanfordnlp.server import CoreNLPClient

tokenizer_client = CoreNLPClient(annotators=['tokenize', 'ssplit'], timeout=30000, memory='16G', properties={'tokenize.ptb3Escaping': False, 'tokenize.options': "splitHyphenated=true,invertible=true", 'ssplit.eolonly': True}, threads=16)
...
def process_file(filename, config, word_counter=None, char_counter=None):
    ...
    #outputs = Parallel(verbose=10)(delayed(_process_article)(article, config) for article in data)
    outputs = [_process_article(article, config) for article in tqdm(data)]
    ...
    if word_counter is not None and char_counter is not None:
        ...
        for example in tqdm(examples):
            # Change 3: Get all the words of all the paragraphs.
            word_counter.update(example['ques_tokens'])
            word_counter.update(word for para in example['context_tokens'] for word in para)
            for token in example['ques_tokens']:
                char_counter.update(token)
            for token in (word for para in example['context_tokens'] for word in para):
                char_counter.update(token)
